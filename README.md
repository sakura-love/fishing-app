...
- 关键词搜索，快速定位目标鱼种
- 自动根据当前水温推荐最佳鱼种
- **自定义鱼种**：拍照或从相册选择照片，添加图鉴中没有的鱼种（v2.0.0）
...
- **智能添加**：识别到图鉴中没有的鱼种时，可一键添加到自定义图鉴（v2.0.0）

### 👤 个人中心
- **自定义昵称**：点击编辑专属昵称（v2.0.0）
- **自定义头像**：支持拍照或从相册选择头像（v2.0.0）
- 丰富的统计：总钓获、鱼种数量、最大鱼记录
- 成就系统：初出茅庐 → 小有斩获 → 鱼种猎人 → 大物猎手
...
│   ├── useCustomFish.ts        # 自定义鱼种状态管理（Zustand）
│   ├── useWeather.ts           # 天气与定位
│   └── useUnits.ts             # 单位偏好管理
├── services/                   # 服务层
...
│   ├── custom-fish.ts          # 自定义鱼种 CRUD
│   └── user-profile.ts         # 用户资料管理
├── utils/                      # 工具函数
...
### v2.0.0（最新）
- **自定义鱼种**：图鉴页面右上角新增「+」按钮，支持拍照或从相册选择照片添加图鉴中没有的鱼种
- **AI 智能添加**：AI 识别到图鉴中没有的鱼种时，自动提示并提供「添加到我的图鉴」选项，预填识别信息
- **自定义昵称**：个人中心点击昵称可编辑专属昵称
- **自定义头像**：个人中心点击头像可拍照或从相册选择头像
- 自定义鱼种支持查看详情、删除管理
- 图鉴列表和详情页同时展示内置鱼种和自定义鱼种
